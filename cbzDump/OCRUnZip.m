...
NSString *const OCRUnZipDomain = @"OCRUnZipDomain";
...
- (instancetype)initWithZipFile:(NSString *)path  error:(NSError * _Nullable *)outErr
{
	...
	if(self) {
		...
		if(!self.unzip) {
			if (outErr) {
				*outErr = [NSError errorWithDomain:OCRUnZipDomain code:CANT_READ_AS_ZIP userInfo:@{
...
- (NSData *)dataWithContentsOfFile:(NSString *)path error:(NSError * _Nullable *)outErr
{
  if (!self.unzip) {
		if (outErr) {
			*outErr = [NSError errorWithDomain:OCRUnZipDomain code:ZIP_NOT_OPEN userInfo:@{
  ...
  if (!path.length) {
		if (outErr) {
			*outErr = [NSError errorWithDomain:OCRUnZipDomain code:ZIP_PATH_IS_NIL userInfo:@{
  ...
  if(unzLocateFile(self.unzip, rawFilename, 0) != UNZ_OK) {
		if (outErr) {
			...
			*outErr = [NSError errorWithDomain:OCRUnZipDomain code:CANT_LOCATE_SUBFILE userInfo:@{
  ...
  if(unzOpenCurrentFile(self.unzip) != UNZ_OK) {
		if (outErr) {
			...
			*outErr = [NSError errorWithDomain:OCRUnZipDomain code:CANT_OPEN_SUBFILE userInfo:@{
				NSLocalizedDescriptionKey : s
			}];
		}
    return nil;
    ...
    if(length == 0) {
      break;
			...
			if (outErr) {
				...
				*outErr = [NSError errorWithDomain:OCRUnZipDomain code:CANT_READ_SUBFILE userInfo:@{
					NSLocalizedDescriptionKey : s
				}];
			}
